		...
		<div class="inputNav">
			<div class="forms">
				<el-select v-model="region.building.buildingId" :disabled="buildingSelectFlag" placeholder="请选择写字楼" @change="getInfoByBuildingId">
					<el-option v-for="building ,index in region.buildingList" :key="index" :label="building.buildingName" :value="building.buildingId"></el-option>
				</el-select>
				<el-select v-model="region.floor.floorId" clearable :disabled="floorSelectFlag" placeholder="请选择楼层" @clear="clearFloorSearch" @change="changeInfoByFloorId">
					<el-option v-for="floor ,index in region.floorList" :key="index" :label="floor.buildingFloorName" :value="floor.buildingFloorId"></el-option>
				</el-select>
			</div>
		</div>
		<div class="clear20"></div>
		<div class="buildingFloor" v-if="isShowRoomChose&&isShowRoomChoseValue">
			<div class="floorUnit" v-for="floor,index in getRoomFromServer">
				...
				<div class="floor fr" :ref="'roomHeight_'+floor.floorName">
					<div class="clear20"></div>
					<div class="oldRoom">
						<el-tag v-for="tag,index in floor.roomList" :key="index" :disable-transitions="false" :choseFlag="tag.choseFlag" color="#009688">
					...
					<div class="newRoom" v-if="isCanAddRoomFun">
						<el-tag v-for="tag,index in roomTagsNew[floor.buildingFloorId]" :key="index" closable :disable-transitions="false" :hoverFlag="tag.hoverFlag" color="#e39e52" @close="handleClose(roomTagsNew[floor.buildingFloorId],floor,tag)">
							<!--{{tag.roomName}}-->
							<span class="innerSpan">{{tag.roomName}}</span>
							<!-- @mouseenter="newRoomEnter(tag)"  @mouseleave="newRoomLeave(tag)"-->
						</el-tag>
						<el-input class="input-new-tag" v-if="allFlag&&inputVisibleInputFlag[floor.buildingFloorId]" v-model="inputValue.roomName" ref="saveTagInput" size="small" @keyup.enter.native="handleInputConfirm(floor)" @blur="handleInputConfirm(floor)">
						</el-input>
...
<script>
    ...
    props:{
      ...
      interfaceString:{
        type:String,
        default:'newBuilding/getRoomList'
      },
      ...
      componentStorage:{
		...
		methods: {
          ...
          if(this.roomParamsToServer.indexOf(tag)!=-1){
			...
			//根据SHOPID获取写字楼
			getBuildingListByCompanyId(callBack) {
				...
				this._httpPasser(
					...
					(res) => {
						...
						if(this.componentStorage!==null&&this.componentStorage.buildingId!==''){
						  let temp = []
						  for(let o in this.region.buildingList){
						    if(this.region.buildingList[o].buildingId==this.componentStorage.buildingId){
						      temp.push(this.region.buildingList[o])
                }
              }
              this.region.buildingList = JSON.parse(JSON.stringify(temp))
            }
        ...
        console.log('aaa',this.interfaceString)
				//获取房间数据
				this._httpPasser(
          this.interfaceString,
					'get', {
						buildingId: this.region.building.buildingId,
            shopId:this.componentStorage!==null?this.componentStorage.shopId:'',
            buildingRoomId:this.componentStorage!==null?this.componentStorage.buildingRoomId:''
					},
					(res) => {
						...
						console.log('房间列表', this.getRoomFromServer)
						/*this.flagForResetHeight = false;
						//根据房间高度，动态设置楼层高度。
						this.$nextTick(() => {
							let ref = this.$refs;
			...
			handleSubmit() {
				...
				re.componentStorage = this.componentStorage;
				console.log('结果', re)
		...
		created() {
			...
			if(this.buildingFloorRoomData!==null && this.buildingFloorRoomData.length > 0 && this.buildingFloorRoomData[0].buildingId) {

				this.getBuildingListByCompanyId(() => {
					//如果有传入的参数，则进行页面渲染
					if(this.buildingFloorRoomData) {
						let roomInfoList = null
						if(this.buildingFloorRoomData.buildingList) {
							roomInfoList = this.buildingFloorRoomData.buildingList[0]
						} else if(this.buildingFloorRoomData.length) {
							roomInfoList = this.buildingFloorRoomData[0]
						} else {
							roomInfoList = this.buildingFloorRoomData
						}

						//写入写字楼数据
						this.region.building.buildingId = roomInfoList.buildingId;
						...
						this.getRoomDataByBuildingId(() => {
							...
							let editFloorListData = roomInfoList.floorList;
							//首先将服务器返回额编辑数据整理为组件返回值形式的数组
							let tempFloor = {};
							for(let o in editFloorListData) {
								tempFloor[editFloorListData[o].buildingFloorId] = [];
								for(let p in editFloorListData[o].roomList) {
									let tempReData = {};
									tempReData.buildingFloorName = editFloorListData[o].buildingFloorName;
									tempReData.buildingFloorId = editFloorListData[o].buildingFloorId;
									tempReData.buildingRoomId = editFloorListData[o].roomList[p].buildingRoomId;
									tempReData.roomName = editFloorListData[o].roomList[p].buildingRoomName;
									tempFloor[editFloorListData[o].buildingFloorId].push(editFloorListData[o].roomList[p].buildingRoomId)
									this.roomParamsToServer.push(tempReData)
								}
							}

							for(let o in this.getRoomFromServer) {
								if(tempFloor[this.getRoomFromServer[o].buildingFloorId]) {
									let tempFloorRoomList = tempFloor[this.getRoomFromServer[o].buildingFloorId];
									for(let p in this.getRoomFromServer[o].roomList) {
										for(let q in tempFloorRoomList) {
											if(this.getRoomFromServer[o].roomList[p].buildingRoomId == tempFloorRoomList[q]) {
												this.getRoomFromServer[o].roomList[p].choseFlag = 'Y'
											}
										}
									}
								}
