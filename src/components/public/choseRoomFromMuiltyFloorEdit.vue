...
<script>
  export default {
    ...
    methods: {
      ...
      //根据区ID获取写字楼列表
      getBuildingListByCountryId(callBack){
        this._httpPasser(
     ...
     // this.getProvince();
      ...
      if(this.buildingFloorRoomData && this.buildingFloorRoomData.length && this.buildingFloorRoomData.length>0){
        this.getProvince(()=>{
          ...
          //写入省数据
          this.region.province.code = roomInfoList.provinceCode;
          //查询市
          this.getCityByProvince(()=>{
            //写入市数据
            this.region.city.code = roomInfoList.cityCode;
            this.getCountryByProvince(()=>{
              //写入区数据
              this.region.country.areaId = roomInfoList.areaId;
              //根据区ID查询写字楼
              this.getBuildingListByCountryId(()=>{
                //写入写字楼数据
        ...
        //console.log(this.buildingFloorRoomData)
      }
      else{
        this.getProvince();
      }
