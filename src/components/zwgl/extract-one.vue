  ...
  <div class="withdraw-contain">
    <div class="withdraw-width">
      ...
      <div class="withdraw-content-head">
        <span class="withdraw-card" @click="test">提取余额到银行卡</span>
      ...
      <div class="withdraw-body">
        ...
        <div class="clearfix withdraw-num">
          <span class="section-left fl withdraw-num1">提现金额：</span>
          <div class="fl">
            <el-input v-model.trim.number="money" placeholder="输入金额" ></el-input>
          </div>
        ...
        <div class="clearfix withdraw-num">
          ...
          <div class="fl">
            <el-input v-model.trim.number="password" type="password" placeholder="请输入6位数密码" ></el-input>
          </div>
        </div>
      ...
      <div class="withdraw-footer">
        <div class="btn-style">
          ...
          <!--<router-link :to="{name:'YgExtractTwo'}">测试下一步需删除</router-link>-->
        </div>
      </div>
      <div class="question">
  ...
  export default {
        ...
        if(this.radio==''){
          ...
          this.$http2.post('asset/transaction',{
            ...
            .then(res=>{
              ...
              console.log(data)
              if(data.resultCode=='200'){
                this.$router.push({path:'extract-two'})
                this.$message({
                  showClose: true,
                  message: '提现成功',
                  type: 'success'
    ...
    beforeRouteEnter(to, from, next){
      next(vm=>{
        ...
        if(isName=='false'){//未实名认证不能 提现 去实名认证页面
          vm.$router.push({path:'nameConfirm'})
        }else if(isBand=='false'){//未设置交易密码不能 提现 去设置交易密码页面
          vm.$router.push({path:'setPaw'})
        }else if(isBanks=='false'){//未绑卡不能 提现 去绑卡页面
          vm.$router.push({path:'bindcard-one'})
        }
      })
    }
  }
