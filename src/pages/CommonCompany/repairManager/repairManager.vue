	...
	<div id="repairList">
		<div class="main-cont">
			<div class="common">
				<div class="top-title">
					<div class="line"></div>
					<span class="top-text">报修单列表</span>
				</div>
			</div>
			<div class="main-box">
				<div class="forms-cont clearfix">
					...
					<div class="form-item fl">
						<el-select v-model="statusId" placeholder="请选择维修状态">
							<el-option v-for="item in repairState" :key="item.statusId" :label="item.statusName" :value="item.statusId">
							</el-option>
						</el-select>
					</div>
					<div class="form-item fl">
                ...
                <div style="color: #000000;font-size: 16px;padding-bottom: 10px;">给报修人的回复：</div>
								<div class="item-ipt fl">
									<el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4}" placeholder="" v-model="dispatch.replyMessage" style="width: 515px;" clearable>
									</el-input>
								</div>
							</div>
							<div class="items clearfix">
                <div style="color: #000000;font-size: 16px;padding-bottom: 10px;padding-top: 10px">请选择维修人员进行派单：</div>
								...
								<div class="item-ipt fl">
									<el-input v-model.trim="getEmployeeParams.userName" placeholder="请输入姓名" clearable></el-input>
								</div>
								<div class="item-text fl" style="width: 42px;margin-left: 12px">
									<el-button type="primary" @click="getEmployeeList">搜索</el-button>
								</div>
							</div>
							<el-table :data="dispatch.employeeList" style="width: 100%" @selection-change="handleSelectionChangeEmployee" :empty-text=text1>
				...
				<el-dialog title="关闭报修单" :visible.sync="closeReport.closeReportDialog" width="640px">
					<div class="form-item clearfix">
						<div class="items clearfix">
							...
							<div class="item-ipt fl">
								<el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4}" placeholder="请输入内容" v-model="closeReport.closeMessage" style="width: 515px;" clearable>
								</el-input>
							</div>
						</div>
						<div slot="footer" class="dialog-footer fr" style="padding-top: 40px">
	...
	export default {
		...
		data() {
			return {
				text:'正在加载，请稍候',
				text1:'正在加载，请稍候',
				companyId: this.$local.sefetch('info') ? this.$local.sefetch('info').companyId : '',//自己公司的id
        propertyCompanyId:'',//物业公司的id
				repairStartTime: '',
      ...
      this.getPositionList()//获取职位
		...
		computed: {},
		methods: {
      getPropertyMessage(){//获取物业公司的基本信息，主要是取到物业公司的id
        this.$httpPasser.post("propertyCompany/getPropertyCompanyInfoByTenantShopId",{
          shopId:this.companyId
        }).then((res)=>{
          this.propertyCompanyId = res.data.result.shopId
          ...
          if(this.positionList[o].customerPostId== id){
			...
			getRepairDocumentsList() { //获取维修列表
				this.$httpPasser.get("repairDocuments/getRepairDocumentsList", {
					params: {
						shopId: this.propertyCompanyId ,
            businessShopId:this.companyId,
