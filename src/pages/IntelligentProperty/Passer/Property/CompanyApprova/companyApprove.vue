<template>
  <div class="mainPart" id="companyApprove">
    <div>
      <div class="top-title">
        <div class="line"></div>
        <span class="top-text">入驻公司审批</span>
      </div>
    </div>
    <el-input
      clearable
        ...
        <div class="clearfix">
          <div class="fl">
            <el-pagination
              ...
              :page-count="companyApprovalInfo.pageTotal">
            </el-pagination>
          </div>
          <div class="fl pagestyle font12">
            <span>共{{this.companyApprovalInfo.pageTotal}}页，当前第{{this.companyApprovalInfo.pageIndex}}页,每页30条，共{{this.companyApprovalInfo.totalRecord}}条</span>
          </div>
        </div>
      </div>
      <div class="fr">
        <el-button type="primary" round @click="handleApprovalBatch('Y')">审批通过</el-button>
        <el-button type="primary" round @click="handleApprovalBatch('N')">审批不通过</el-button>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      indexMethod(index) {//编号
        return (this.companyApprovalInfo.pageIndex - 1)*30 + 1 + index;
      },
      //获取并处理子模块回传的数据
      ...
      enterCompanyApprovalPending() {
        this._httpPasser(
          ...
          this.searchParams,
          data => {
            this.companyApprovalInfo = data;
            this.companyApprovalInfo.companyApprovalList = data.result
          },
          false,
          ()=>{
            this.companyApprovalInfo.pageTotal = 0;
            this.companyApprovalInfo.totalRecord = 0;
            this.text = '暂无数据'
            delete(this.companyApprovalInfo.companyApprovalList)
            this.$set(this.companyApprovalInfo,'companyApprovalList',[])
          }
        )
      },
      //修改表格选项的钩子方法
    ...
    created: function () {
      this.enterCompanyApprovalPending();
      this.companyApprovalInfo.companyApprovalList = [];
    }
  }
</script>
