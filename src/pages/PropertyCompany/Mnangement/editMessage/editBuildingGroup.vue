<template>
  <div id="editBuildingGroup">
    ...
    <div class="back">
      <el-button type="text" icon="el-icon-arrow-left" @click="backHandle">返回</el-button>
      <span style="margin-left:20px;font-size:15px;">一键招商</span>
    </div>
    <div class="clear10"></div>
    <el-form :model="buildingGroupInfo" :rules="rules" ref="buildingGroupInfo" label-width="100px" class="demo-ruleForm">
      <div class="left">
        ...
        <el-form-item label="竣工时间" prop="buildYear">
          <el-date-picker v-model="buildingGroupInfo.buildYear" type="year" format="yyyy" value-format="yyyy" placeholder="竣工时间">
          </el-date-picker>
        </el-form-item>
      ...
      <div class="right">
        ...
        <el-form-item label="开盘时间" prop="stayInYear">
          <el-date-picker v-model="buildingGroupInfo.stayInYear" type="year" format="yyyy" value-format="yyyy" placeholder="开盘时间">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="层高" prop="floorHeight">
          <el-input placeholder="层高" v-model="buildingGroupInfo.floorHeight">
      ...
      <div style="width:100%;" class="formFooter">
        ...
        <el-form-item label="周边环境">
          <el-upload :action="importFileUrl" :data="upLoadData" list-type="picture-card" :file-list="peripheryPic" :on-success="peripheryPicSubmit" :on-remove="peripheryPicDelete" multiple>
        ...
        <el-form-item label="楼盘外观">
          <el-upload :action="importFileUrl" :data="upLoadData" list-type="picture-card" :file-list="groupPic" :on-success="groupPicSubmit" :on-remove="groupPicDelete" multiple>
            <i class="el-icon-plus"></i>
          </el-upload>
        </el-form-item>
        <div class="formButton">
          <el-button type="primary" @click="handleSubmitBuildingGroup('buildingGroupInfo')">保存</el-button>
          <!--<el-button type="primary" @click="handleSubmitBuilding('buildingGroupInfo')">下一步</el-button>-->
          <!-- <el-button>重置</el-button>-->
        </div>
      </div>

      <el-dialog title="选择楼盘所在区域" :visible.sync="dialogVisibleRegion" width="40%" top="22vh" :close-on-click-modal="false">
        <div class="house">
          <div class="forms">
            ...
            <el-select v-model="region.city.code" placeholder="请选择市" :disabled="citySelectFlag" @change="getCountryByProvince">
              <el-option v-for="city ,index in region.cityList" :key="index" :label="city.name" :value="city.code"></el-option>
            </el-select>
            <el-select v-model="region.country.code" :disabled="countrySelectFlag" placeholder="请选择区" @change="changeRegionFlag">
              <el-option v-for="country ,index in region.countryList" :key="index" :label="country.name" :value="country.code"></el-option>
            </el-select>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      backHandle() {
//        this.$router.back()
        this.$router.push("../onePointBusiness")
      },
      //返回楼盘列表方法
      toBuildingGroupList() {
        this.$router.push({
        ...
        this._httpPasser(
          ...
          (data) => {
                      for(let o in data.result){
                        ...
                        if(data.result.manageCompany == ''||data.result.manageCompany ==null){
                          this.buildingGroupInfo.manageCompany = this.$local.sefetch('info').companyName;
                        }
                      }
            ...
            this.buildingGroupInfo.structureType = data.result.structureType;
