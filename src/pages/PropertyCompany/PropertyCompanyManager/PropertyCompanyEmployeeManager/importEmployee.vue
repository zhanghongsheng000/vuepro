<template>
  <div class="mainPart importRenter">
    ...
    <div slot="tip" class="el-upload__tip" style="float: left;">只能上传.xls/.xlsx文件</div>
    <el-dialog
      title="导入结果"
...
<script>
  ...
  export default {
    data() {
      return {
        currentName: "导入员工",
        action: $gPasserUrl + 'employee/importPropertyEmployee',
        //产品ID
    ...
    created: function () {
      //根据产品ID修改当前导入租客调用的地址
      this.action = $gPasserUrl + 'employee/importPropertyEmployee';
      /*if(this.productId=='5'){
