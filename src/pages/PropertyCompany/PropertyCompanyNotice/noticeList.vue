<template>
	<div class="propertyNoticeList">
		<div class="main-cont">
			<div class="line"></div>
			<span class="top-text">已发通知列表</span>
		</div>
		<div class="main-box">
			<div class="forms-cont clearfix">
				...
				<div class="form-item fl" style="width: 70px;">
					<el-button type="primary" @click="toNoticeSend">发通知</el-button>
				</div>
			</div>
		</div>
		<div class="tables">
			<el-table :data="noticeList" style="width: 100%" :empty-text=text>
				...
				<el-table-column label="操作">
					<template slot-scope="scope">
						<el-button size="mini" type="primary" @click="toPropertyCompanyNoticeDetails(scope.$index,scope.row)" round>详情</el-button>
...
<script>
	...
	export default {
		...
		methods: {
			toPropertyCompanyNoticeDetails(index, row) { //跳转详情
				this.$router.push({
					path: 'noticeDetails',
					query: {
			...
			getCompanyNoticeList(index) { //获取物业公司已发送消息列表
			...
			deletesure(row) { //删除消息
				this.$httpPasser.post("message/deleteNotice", {
					noticeId: row.noticeId
				}).then((res) => {
					if(res.data.resultCode == '200') {
						this.$message.success("删除成功");
						this.getCompanyNoticeList(this.page.currentPageNum) //加载列表
					} else {
						this.$message({
							showClose: true,
							message: res.data.resultCodeDesc,
